...
(function () {
  ...
  function checkTwitchUserStatus(username) {
    const clientId = 'kimne78kx3ncx6brgo4mv6wki5h1ko'
    const query = `query($login: String!) { user(login: $login) { id login banned suspended } }`
    return new Promise(resolve => {
      GM.xmlHttpRequest({
        method: 'POST',
        ...
        data: JSON.stringify({ query, variables: { login: username } }),
        onload: res => {
          ...
          if (res.status === 200) {
            try {
              ...
              if (!user) {
                ...
                const banned = user.banned || user.suspended
                console.log('checkTwitchUserStatus banned?', banned)
                resolve({ exists: true, banned: Boolean(banned) })
              }
            } catch (e) {
